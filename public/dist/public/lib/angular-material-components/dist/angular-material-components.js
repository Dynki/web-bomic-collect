...
.controller('mdcDatePickerController', function ($scope, $timeout, $mdDialog, $document, model, locale, mdTheme) {
    ...
    $scope.displayMonthSelection = function () {
        //var calendarHeight = $document[0].getElementsByClassName('mdc-date-picker__calendar')[0].offsetHeight;
        //var monthSelectorElement = $document[0].getElementsByClassName('mdc-date-picker__month-selector')[0];
        //monthSelectorElement.style.height = calendarHeight + 'px';

        $scope.monthSelection = true;

        $timeout(function () {
            var activeMonthElement = $document[0].getElementsByClassName('mdc-date-picker__month--is-active')[0];
            monthSelectorElement.scrollTop = monthSelectorElement.scrollTop + activeMonthElement.offsetTop - monthSelectorElement.offsetHeight / 2 + activeMonthElement.offsetHeight / 2;
        });
    };
...
.controller('mdcDatePickerInputController', function ($scope, $attrs, $timeout, $mdDialog) {
    ...
    $scope.openPicker = function (ev) {
      ...
      $mdDialog.show({
        targetEvent: ev,
        templateUrl: 'modules/core/views/date-picker-dialog.html',
...
.directive('mdcDatePicker', function () {
    return {
      ...
      templateUrl: 'modules/core/views/date-picker-input.html'
    };
  });
})();
(function(){'use strict';
