    ...
    function($resource, settings, $q, $rootScope, pouchDB) {
        ...
        function loadLookupArrays(){
            ...
            settings.$get().$promise.then(function(apiLocation){
                ...
                eventTypesService.query().$promise.then(function(apiResults){
                        ...
                        if (results.rows.length > 1) {
                            ...
                            db.bulkDocs(results.rows).then(function (result) {
                                ...
                                angular.forEach(apiResults, function(resultItem){
                                    ...
                                    var updatedDocs = [];
                                    var item = { _id: 'EventType' + resultItem.etype1, table: 'EventTypes', keyvalue: resultItem.etype1, text: resultItem.name};
                    ...
                    db.allDocs({
                        ...
                        angular.forEach(results.rows, function(resultItem){
                            ...
                            var arrayItem = { id: resultItem.doc.keyvalue, text: resultItem.doc.text};
                            resultArray.push(arrayItem);
                        });
                ...
                // Load any $resource services and query their data to load a static list into an array attached to the lookupData object.
                var eventSubTypesService = $resource( apiLocation + '/EventSubTypes', {},{query: { method: 'GET', params: {}, isArray: true }});
                        ...
                        if (results.rows.length > 1) {
                            ...
                            db.bulkDocs(results.rows).then(function (result) {
                                ...
                                angular.forEach(apiResults, function(resultItem){
                                    ...
                                    var updatedDocs = [];
                                    var item = { _id: 'EventSubType' + resultItem.etypesub, table: 'EventSubTypes', keyvalue: resultItem.etypesub, text: resultItem.name};
                                    updatedDocs.push(item);
                                    db.bulkDocs(updatedDocs);
    
                                    var arrayItem = { id: resultItem.etypesub, text: resultItem.name};
                                    resultArray.push(arrayItem);
                                });
                        ...
                        if (results.rows.length > 1) {
                            ...
                            db.bulkDocs(results.rows).then(function (result) {
                                ...
                                angular.forEach(apiResults, function(resultItem){
                                    ...
                                    var updatedDocs = [];
                                    var item = { _id: 'EventStatus' + resultItem.estatus1, table: 'EventStatus', keyvalue: resultItem.estatus1, text: resultItem.name};
                                    updatedDocs.push(item);
                                    db.bulkDocs(updatedDocs);
                        ...
                        if (results.rows.length > 1) {
                            ...
                            db.bulkDocs(results.rows).then(function (result) {
                                ...
                                angular.forEach(apiResults, function(resultItem){
                                    ...
                                    var updatedDocs = [];
                                    var item = { _id: 'EventLocation' + resultItem.evloc1, table: 'EventLocations', keyvalue: resultItem.evloc1, text: resultItem.name};
                                    updatedDocs.push(item);
                                    db.bulkDocs(updatedDocs);
                        ...
                        if (results.rows.length > 1) {
                            ...
                            db.bulkDocs(results.rows).then(function (result) {
                                ...
                                angular.forEach(apiResults, function(resultItem){
                                    ...
                                    var updatedDocs = [];
                                    var item = { _id: 'Keyworker' + resultItem.keyw1, table: 'Keyworkers', keyvalue: resultItem.keyw1, text: resultItem.name};
                                    updatedDocs.push(item);
                                    db.bulkDocs(updatedDocs);
